...
```bash
weav urls script.js
weav urls --input script.js
cat script.js | weav urls
```
...
```bash
weav urls script.js --placeholder UNKNOWN
```
...
```bash
weav urls script.js --include-templates
```
...
```bash
weav urls script.js --output results.txt
```
...
```bash
weav urls large-bundle.js --max-nodes 5000000
```
...
```bash
weav urls bundle.js --max-file-size 2.0
```
...
```bash
weav urls script.js --html-parser html5lib
```
...
```bash
weav urls script.js --skip-symbols
```
...
```bash
weav urls script.js --skip-aliases
```
...
```bash
weav urls script.js --extensions proto,protobuf
```
...
**JSON string:**
```bash
weav urls script.js --context '{"API_BASE":"https://api.example.com"}'
```

**KEY=VALUE pairs:**
```bash
weav urls script.js --context 'API_BASE=https://api.example.com,CDN=https://cdn.com'
```

**JSON file:**
```bash
echo '{"API_BASE":"https://api.example.com"}' > context.json
weav urls script.js --context context.json
```
...
```bash
weav urls script.js --context 't=/api' --context-policy merge
```
...
```bash
weav urls script.js --context 't=/api' --context-policy override
```
...
```bash
weav urls script.js --context 't=/api' --context-policy only
```
...
```bash
weav urls app.js --context '{"BASE_URL":"https://api.example.com"}' --include-templates
```

### Override window.location.host
```bash
weav urls app.js --context 'window.location.host=example.com'
```

### Process large bundle efficiently
```bash
weav urls bundle.js --max-file-size 5.0 --skip-aliases
```

### Debug variable resolution
```bash
weav urls app.js --include-templates --verbose
```

### Extract to file
```bash
weav urls app.js --include-templates --output urls.txt
```
...
```bash
weav urls bundle.js --max-nodes 10000000 --skip-aliases
```

### Finding API endpoints in React apps
```bash
weav urls app.js --context '{"API_URL":"https://api.example.com"}' --include-templates
```

### Security testing
```bash
weav urls app.js --include-templates | grep -E "https?://"
```
